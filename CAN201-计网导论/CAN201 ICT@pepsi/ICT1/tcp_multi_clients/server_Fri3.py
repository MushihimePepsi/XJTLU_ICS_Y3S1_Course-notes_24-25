...
usr_num = 0
usr_records = []  # address, name, answer, socket
turn_num = -1
current_answer = -1
sorted_array = []
...
def set_usr_answer(name, answer):
    """设定用户的猜数"""
    global usr_records
    for usr in usr_records:
        if usr[1] == name:
...
# def get_farthest_usr():
#     """得到上一局离答案最远的用户"""
#     global usr_records, random_number, farthest_usr
#     distance = -1
#     for usr in usr_records:
#         cur_distance = abs(random_number - usr[2])
...
def game_service():
    """用于提供游戏服务的方法"""
    global turn_num, current_answer, usr_records, this_socket, sorted_array
    while True:
        if turn_num > -1:
            send_json_everyone('start', -1, -1)
            while True:
                if turn_num < len(usr_records): # 第一轮，大家按进房顺序猜数
                    turn_name = usr_records[turn_num % len(usr_records)][1]
                else: # 之后的轮次，每次都让最远的人猜数
                    if turn_num % len(usr_records) ==0:
                        sorted_array = sorted(usr_records, key=lambda x: abs(x[2] - random_number), reverse=True)
                    turn_name = sorted_array[turn_num % len(usr_records)][1]
                ...
                for usr in usr_records:
                    if usr[1] == turn_name:
                        this_socket = usr[3]
                        break
                time.sleep(0.3) # 防止粘包！！！在win上不会粘，但linux会粘，可以写个分包的方法，但我太懒就等一会吧~
                send_json('turn', turn_name, -1, this_socket)  # 发信求猜
                that_client_answer = this_socket.recv(20480) # 收信判断
                answer = int(read_json(that_client_answer).get('answer'))  # 收到答案，然后判断发给每个人
                set_usr_answer(turn_name, answer)  # 记录用户猜数
...
def TCP_processor(connection_socket, address):
    ...
    try:
        ...
        usr_records.append([address, cl_name, -1, connection_socket]) # address, name, answer, socket
