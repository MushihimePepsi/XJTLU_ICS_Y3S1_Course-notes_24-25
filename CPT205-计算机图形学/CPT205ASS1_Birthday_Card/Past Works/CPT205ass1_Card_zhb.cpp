...
{   // keyboard interactions
	if (key == 'q' || key == 'Q')
		...
		coverStep = 0;
	else if (key == 'f' || key == 'F') // remove the coverTime
		coverStep = 3;
	else if (key == 'b' || key == 'B') // move back the coverTime
		coverStep = -3;
	else if (key == 'r' || key == 'R') // reset the coverTime
	{
		pCoverUp = { 0, h };
		pCoverDown = { 0, 0 };
	}
...
void drawLogo() 
{	// A long series of gl function to draw the logo on the coverTime
	glPushMatrix();
	glTranslatef(0, -40, 0);
...
void mouse_input(int button, int state, int x, int y) 
{
	...
	else if (state == GLUT_DOWN && button == GLUT_MIDDLE_BUTTON) // click down to draw a bigger star
	{	
...
void menu(int item) //mouse and keyboard can control the display altogether
{
	...
	mouse_input((unsigned char)item, GLUT_DOWN, w/2, h/2);
	SpecialKey((unsigned char)item, 0, 0);
}
////////////////////////////////
int main(int argc, char** argv)
{
	...
	int cover, zoom, generate;
	cover = glutCreateMenu(menu);
	glutAddMenuEntry("Open the Card [F]", 'f');
	glutAddMenuEntry("Close the Card [B]", 'b');
	glutAddMenuEntry("Stop the Cover [S]", 's');
	glutAddMenuEntry("Reset the Cover [R]", 'r');

	zoom = glutCreateMenu(menu);
	glutAddMenuEntry("Move Left [LEFT]", GLUT_KEY_LEFT);
	glutAddMenuEntry("Move Right [RIGHT]", GLUT_KEY_RIGHT);
	glutAddMenuEntry("Move Up [UP]", GLUT_KEY_UP);
	glutAddMenuEntry("Move Down [DOWN]", GLUT_KEY_DOWN);
	glutAddMenuEntry("Shrink the Viewport [W]", 'w');
	glutAddMenuEntry("Enlarge the Viewport [E]", 'e');

	generate = glutCreateMenu(menu);
	glutAddMenuEntry("Generate a Small Star [Left Click]", GLUT_LEFT_BUTTON);
	glutAddMenuEntry("Generate a Big Star [Middle Click])", GLUT_MIDDLE_BUTTON);

	glutCreateMenu(menu);
	glutAddMenuEntry("Control Menu", '\0');
	glutAddMenuEntry("------------------------", '\0');
	glutAddSubMenu("Cover Control ", cover);
	glutAddMenuEntry(" ", '\0');
	glutAddSubMenu("Adjust the Viewport ", zoom);
	glutAddMenuEntry(" ", '\0');
	glutAddSubMenu("Generator ", generate);
	glutAddMenuEntry(" ", '\0');
	glutAddMenuEntry(" ", '\0');
	glutAddMenuEntry("Thank you for reading My Card", '\0');
	glutAddMenuEntry("------------------------", '\0');
